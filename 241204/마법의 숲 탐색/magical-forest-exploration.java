...
public class Main {
	...
	static boolean canMoveDown(int r, int c) {
		for (int i=1; i<4; i++) {
			int nr = r + dr[i] + 1;
			int nc = c + dc[i];
			if (nr<-2 || nc<=0 || nr>R || nc>C) return false;
	...
	static boolean isInForest(int r, int c) {
		if (r <= 0) return false;
		for (int i=0; i<4; i++) {
			int nr = r + dr[i];
			int nc = c + dc[i];
	...
	static int moveFairy(int r, int c) {
		...
		visited[r][c] = true;
		q.add(new Fairy(r,c,forest[r][c]));
		while (!q.isEmpty()) {
			Fairy f = q.poll();
			...
			for (int i=1; i<4; i++) {
				...
				if (forest[nr][nc] == f.g) {
					visited[nr][nc] = true;
					q.add(new Fairy(nr,nc,f.g));
				}
				else {
					if (isExit[f.r][f.c]) {
						visited[nr][nc] = true;
						q.add(new Fairy(nr,nc,forest[nr][nc]));
					}
				}
			}
		}
